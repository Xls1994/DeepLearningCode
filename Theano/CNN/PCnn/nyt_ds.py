...
def train_conv_net(train,
    ...
    while (epoch < epochs):
        ...
        np.savetxt(str(epoch)+'_test.txt',test_eposs,fmt="%.4f",delimiter=' ')
...
def loadPickle():
    print('load files...')
    path ='gap_40_len_80/train.p'
    f =cPickle.load(open(path))
    print f


if __name__ == "__main__":
    epochs, static, hidden_units_str, batch_size,\
    window_size, conv_non_linear, dimension, inputdir, norm = loadParameters()
